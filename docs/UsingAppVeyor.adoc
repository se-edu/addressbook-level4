...
+
.  Update the link to the "build type" badge at the top of `README.adoc` to point to the AppVeyor build type of your own repo.
* To find your build type badge URL, first go to your project settings by clicking on the "Settings" icon:
+
...
+
----
[![Build type](<link to image>)](<link to project>)
----
+
Convert it to the asciidoc format as follows:
+
----
<link to project>[image:<link to image>[Build type]]
----
+
The asciidoc code should look similar to:
+
----
https://ci.appveyor.com/project/damithc/addressbook-level4[image:https://ci.appveyor.com/api/projects/type/3boko2x2vr5cc3w2?svg=true[Build type]]
----
+
