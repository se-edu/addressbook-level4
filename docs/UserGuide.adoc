...
Format: `add n/NAME p/PHONE_NUMBER e/EMAIL a/ADDRESS [t/TAG]...` Alias: `a`
...
Format: `edit INDEX [n/NAME] [p/PHONE] [e/EMAIL] [a/ADDRESS] [t/TAG]...` Alias: `e`
...
* `export calendar` +
Exports the data on the calendar to a .ics file
* `export portfolio` +
Exports the data in the portfolio to a .csv file

[NOTE]
====
* Coming in v2.0, there will be more options on the types of data to export, since as of v1.2 the only export types are CALENDAR and PORTFOLIO.
* As of v1.2, the application only handles the command without actually exporting. The actual export of the data files will come in v2.0.
====
