...
*API* :
link:{repoURL}/src/main/java/seedu/address/logic/commands/FindCommand.java[`FindCommand.java`]

. `FindCommand` uses `NameContainsKeywordsPredicate` and passes it to `Model` to filter the list of persons in the address book by name.
. `NameContainsKeywordsPredicate` stores a list of keywords, and returns true if any of the individual words in the person's name matches any of the keywords.
