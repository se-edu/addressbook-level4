...
Select `Plugins`. Press `Browse Repository`, and find the plugin. Restart the IDE to complete the installation.
...
==== Configuring the import statement

IntelliJ uses a different import order from the coding style. To rectify that issue, go to `File` > `Settings...` (Windows/Linux), or `IntelliJ IDEA` > `Preferences...` (macOS).
Select `Editor` > `Code Style` > `Java`. Go to the `Imports` tab to set the order.
