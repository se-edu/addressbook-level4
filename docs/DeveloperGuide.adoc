...
.  *GUI Tests* - These are _unit tests_ targeting the individual components of the GUI and _system tests_ that test the entire App by simulating user actions on the GUI. These are in the `seedu.address.ui` and `systemtests` package.
