...
=== Task Feature
==== Implementation
* The task feature is facilitated by `VersionedTaskList`.
 It is stored internally as an TaskListStateList and currentStatePointer.
 It consists of 4 components:
** `VersionedTaskList#AddTask` - Adds a new task to the Student Hand Book
** `VersionedTaskList#listTask` - Lists down the existing Tasks
** `VersionedTaskList#editTask` - Edits an existing Tasks
** `VersionedTaskList#deleteTask` - Deletes an existing Task

==== Model
Like the undo/redo feature as listed above, this model of this feature is almost s identical to it, as we felt that we should reuse existing methods and models.
...
The Student HandBook will initialize with the initial task list state and the currentStatePointer
...
the input string and to process it. It also searches for invalid input.

Step 4. Once valid, adds to the ArrayList called TaskList.

==== Why is this implemented this way
As our task is to add on to an existing product (AddressBook), we decided to follow
the same format as AddressBook for this function, as we can reuse some of the code
through OOP concepts.

Also, an ArrayList seemed the most appropriate for a list function.
...
==== Implementation
* The expenditure list feature is facilitated by `VersionedExpenditureList`.
 It is stored internally as an ExpenditureListStateList and currentStatePointer.
 It consists of 4 components:
** `VersionedExpList#AddPurchase` - Adds a new purchase to the Student Hand Book
** `VersionedExpList#listExp` - Lists down the existing expenses
** `VersionedTaskList#deletePurchase` - Deletes an existing Purchase
** `VersionedExpList#clearExp` - Clears the expenditure list
...
* `Model#addPurchase()`
* `Model#listExp()`
* `Model#deletePurchase()`
* `Model#clearExp()`

Given below is an example usage scenario and how the add mechanism behaves at each step.

Step 1. The user launches the application for the first time.
The Student HandBook will initialize with the initial expenditure list state.

Step 2. The user executes the `addPurchase` command with `addPurchase n/Ice Cream p/$1.50`

Step 3. This command goes through the `AddPurchaseCommandParser` to extract the values of
the input string and to process it.

Step 4. Once valid, adds to the ArrayList called ExpenditureList.

==== Why is this implemented this way
Similarly to the task list, we decided to follow the same format as AddressBook for this function, as we can reuse some of the code
through OOP concepts.

Likewise, an ArrayList seemed the most appropriate for a list function.
