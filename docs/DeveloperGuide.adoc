...
|`* * *` |user |add the appointment date I have with my client |keep track of the next meet up
...
|`* *` |user |duplicate my address book |export the address book
...
|`*` |user |encrypt the data |feel safer for my client's data

|`*` |user |request access to other address book |seek potential customer from leaving colleague

|`*` |user |upload my client's info on social network |protect myself against cyber bullying or defaming
...
[none]
* 2a. Person already exists in AddressBook.
+
[none]
...
[none]
* 2a. Person does not exist in the AddressBook.
+
[none]
** 2a1. AddressBook shows error message.
+
...
3. AddressBook shows the added appointment entry
+
Use case ends.
...
2. AddressBook shows commission earned from client.
+
Use case ends.
