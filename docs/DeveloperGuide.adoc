...
* has a need to keep track of a significant number of employees
...
|`* * *`|user |sort employee by name |locate an employee easily

|`* * *`|user |search employees by specific criteria |
...
|`* * *`|user |add a timetable entry on anyone's timetable |

|`* * *`|user |give employees a rating |evaluate their performance

|`* * *`|user |change employees' rating |

|`* * *`|user |sort the employees by their rating |give them bonus salaries accordingly

|`* * *`|user |sort the employees by their salaries |see their salary conditions

|`* * *`|user |add notes on a timetable entry or on a person |

|`* * *`|user |edit a timetable entry |update timetable entry information

|`* * *`|user |delete a timetable entry |
...
|`*`|user |change the window scheme/theme/skin |

|`*`|user |know employees' location |search their location on the map

|`*`|user |lock the addressbook |leave my app open while making unauthorized people cannot access it

|`*` |user |unlock the addressbook |continue to use the app after leaving it locked

|`* *` |user |mass adding timetable entry to many employees' timetable at once

|`* *` |user |be notified for any deadline for the timetable entry of my employees

|`* *` |user |export my addressbook | share information with another user

|`* *` |user |import addressbook | obtain information from another user

|`*` |user |write a review to an employee that has ever worked below me
|=======================================================================
...
[discrete]
=== Use case: Give employees a rating
...
+
[none]
** 4a1. AddressBook updates the rating for the person with the new rating.
+
Use case ends.
...
2. AdressBook accesses the list of all employees
3. AddressBook sorts the employees in the list by their ratings
4. AddressBook shows the sorted list
+
Use case ends.
...
2. AddressBook shows a list of all employees
3. User requests to add timetable entry to many employees
4. AddressBook add the entry to many employees
+
Use case ends.
...
4. AddressBook add the review to the employee
+
Use case ends.
...
1. User requests export AddressBook into a directory
2. AddressBook export the AddressBook data in a file to the directory specified
+
Use case ends.
...
[discrete]
=== Use Case: Import AddressBook

*MSS*

1. User requests import AddressBook file from a directory
2. AddressBook import the AddressBook data from the file in the directory specified
+
Use case ends.
...
3. AddressBook shows the timetable of that person.
4. User requests to update the information of an entry in the timetable.
5. AddressBook updates the new information for that timetable entry.
+
Use case ends.
...
* 5a. AddressBook couldn't write to save file.
[none]
** 5a1. AddressBook shows error message and requests user to resolve the error.
...
5. AddressBook deletes the timetable entry.
+
Use case ends.
...
3. AddressBook opens up a webpage for composing email to that person.
+
Use case ends.
...
3. AddressBook exports the list of persons as excel sheet.
+
Use case ends.
...
** 2a1. AddressBook shows an error message.
** Use case resumes at step 2.
* 3a. AddressBook couldn't write to save file.
[none]
** 3a1. AddressBook shows error message and requests user to resolve the error.
...
1. User starts the AddressBook program.
2. AddressBook requests user to enter username and password.
3. User enters his username and password
4. AddressBook shows the content.
+
Use case ends.
...
Use case: Lock the addressbook
*MSS*
1. User requests to lock the addressbook by entering "lock" and password
2. AddressBook is locked unless user unlocks it.
* 2a. When addressbook is locked, user are required to unlock the addressbook before any instruction.
+
Use case ends.
Use case: Unlock the addressbook
1. User requests to unlock the addressbook by entering "unlock" and the password set earlier
2. AddressBook unlocked. Use case ends.
*Extensions*
* 1a. The given password is incorrect, which means different from the one set earlier
** 1a1. Addressbook shows an error message and requires to re-enter password
+
Use case resumes at step 1
...
4. AddressBook changes the rating of the employee.
+
Use case ends.
*Extensions*
* 3a. The employee does not exist in addressbook.
** 3a1. AddressBook shows an error message.
Use case resumes at step 2.
...
2. AddressBook changes the scheme/theme/skin
+
Use case ends.
_{More to be added}_
