...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `PersonListPanel`, `StatusBarFooter`, `BrowserPanel`, 'ModuleListPanel' etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
[[Design-Layout]]
=== UI component
* The UI is seperated into 3 columns to clearly identify the different outputs from commands entered by the user.
* First UI Column is for Modules that have already been completed
* Second UI Column is for Modules whose grades have been calculated by the application
* Third UI Column is for displaying summary statistics.
...
. The UI is split into four sections; the three display columns and the search bar.

. Column 1

.. Used to display modules that the user has entered which are already completed.
.. It is assumed that the modules in this column have grades that are true representations of the users academic results.

. Column 2

.. Used to display modules that the user wants to predict the grades of in order to reach CAP goal.
.. It is assumed that the modules in this column have not been completed by the user and therefore do not have an entered grade.

. Column 3

.. Used to display summary statistics of the information entered by the user and calculated by the app.
.. Includes total number of modules entered, CAP Goal, and required grades to reach CAP Goal.
