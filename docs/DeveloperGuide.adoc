...
We have prepared some exercises for beginners of the codebase. There are two kinds of exercise: the first one (i.e. improving each component) is for learning one component at a time, and the second one (i.e. adding the remark command) is to learn how to implement an end-to-end feature across all components.
...
. Add a shorthand equivalent alias for each of the individual commands. For example, beside typing `clear`, the user can also type `c` to remove all persons in the list.
+
****
* Hints
** Just like we store each individual command word constant `COMMAND_WORD` inside `*Command.java` (e.g. `FindCommand#COMMAND_WORD`, `DeleteCommand#COMMAND_WORD`), you need a new constant for aliases as well.
** link:{repoURL}/src/main/java/seedu/address/logic/parser/AddressBookParser.java[`AddressBookParser`] is responsible for analyzing command words.
* Solution
** For each `*Command.java`, add a new constant `COMMAND_ALIAS`.
...
** Explore the link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/address/model/person/Person.java[`Person`] classes to see which API methods you can use to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] allows you to update a person, and link:{repoURL}/src/main/java/seedu/address/model/person/Person.java[`Person`] allows you to update the tags.
* Solution
** Add the new method to the link:{repoURL}/src/main/java/seedu/address/model/Model.java[`Model`] interface.
...
****
* Hints
** The tag labels are created inside link:{repoURL}/src/main/java/seedu/address/ui/PersonCard.java[`PersonCard`] (`new Label(tag.tagName)`). https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Label.html[JavaFX's Label class] allows you to modify the style of each Label, such as changing its color.
** Use the .css attribute `-fx-background-color` to add a color.
* Solution
** See this https://github.com/se-edu/addressbook-level4/pull/592/files[PR] for the full solution.
****

. Modify link:{repoURL}src/main/java/seedu/address/commons/events/ui/NewResultAvailableEvent.java[`NewResultAvailableEvent`] such that link:{repoURL}src/main/java/seedu/address/commons/events/ui/ResultDisplay.java[`ResultDisplay`] can show a different style on error (currently it shows the same regardless of errors).
+
**Before**
+
image::getting-started-ui-result-before.png[width="300"]
+
**After**
+
image::getting-started-ui-result-after.png[width="300"]
+
****
* Hints
** link:{repoURL}src/main/java/seedu/address/commons/events/ui/NewResultAvailableEvent.java[`NewResultAvailableEvent`] is raised by link:{repoURL}src/main/java/seedu/address/commons/events/ui/CommandBox.java[`CommandBox`] (which has the result status), and is catched by link:{repoURL}src/main/java/seedu/address/commons/events/ui/ResultDisplay.java[`ResultDisplay`] (which is where we want to change the style to).
** link:{repoURL}src/main/java/seedu/address/commons/events/ui/CommandBox.java[`CommandBox`] has a similar style for displaying an error. It also has methods for setting the style.
* Solution
** Modify link:{repoURL}src/main/java/seedu/address/commons/events/ui/NewResultAvailableEvent.java[`NewResultAvailableEvent`] 's constructor so that users of the event can indicate whether an error has occurred.
** Modify link:{repoURL}src/main/java/seedu/address/commons/events/ui/ResultDisplay.java[`ResultDisplay#handleNewResultAvailableEvent(event)`] to react to this event appropriately.
...
**Before**
+
image::getting-started-ui-status-before.png[width="600"]
+
**After**
+
image::getting-started-ui-status-after.png[width="600"]
+
****
* Hints
** link:{repoURL}src/main/resources/view/StatusBarFooter.fxml[`StatusBarFooter.fxml`] will need a new `StatusBar`. Be sure to set the `GridPane.columnIndex` properly for each `StatusBar` to avoid misalignment!
** link:{repoURL}src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] needs to change the status bar: (1) When the application just started. (2) When the address book is updated.
* Solution
...
. Add a new method `backupAddressBook(ReadOnlyAddressBook)`, so that the address book can be saved in a different temporary location.
+
****
* Hint
...
. Add `RemarkCommandTest` that will test that executeUndoableCommand() throws an Exception.
. Add new test to `AddressBookParserTest`, which will test that typing "remark" returns an instance of `RemarkCommand`.
...
. Add three methods `setRemark(Remark)`, `getRemark()` and `remarkProperty()`. Be sure to implement these newly created methods in `Person`, which implements the `ReadOnlyPerson` interface.
...
. Fix `validAddressBook.xml` such that the XML tests will not fail due to a mising `<remark>` element.
...
. Modify `GuiTestAssert#assertCardDisplaysPerson(...)` so that it will also do comparison for the now-functioning remark label
. In `PersonCardTest`, call `personWithTags.setRemark(ALICE.getRemark())` to
test that changes in the `Person` 's remark affects the appearance of the `PersonCard`.
