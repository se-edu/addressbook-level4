...
[TIP]
Install the plugin by going to `File` > `Settings` > `Plugins`. Press `Browse Repository`, and find the plugin. Restart the IDE to complete the installation.
...
IntelliJ uses a different import order from the coding style. To rectify that issue, go to `File` > `Settings...` > `Editor` > `Code Style` > `Java`. Go to the `Imports` tab to set the order.
