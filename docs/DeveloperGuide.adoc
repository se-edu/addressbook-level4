...
**Problem: GUI Tests fails on macOS Mojave, and keyboard and mouse movements are not being simulated.**

* Reason: From macOS Mojave onwards, applications without `Accessibility` permission cannot simulate keyboard and mouse movements.
* Solution: Open `System Settings`, click `Settings and Privacy` -> `Privacy` -> `Accessibility`, and grant `Accessibility` permission to `Intellij IDEA`.
