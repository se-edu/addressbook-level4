...
. Run the Gradle task `processResources` (see <<UsingGradle#running-gradle-commands, UsingGradle.adoc>> to learn how to execute Gradle's tasks). +
This task will generate a HTML version of the User Guide (from the asciidoc version), to be used by the `HelpWindow`.
. (Optional) You may want to use Gradle to execute tests. This automatically executes Gradle tasks such as `processResources` before running the tests.
.. Go to `File` > `Settings` (Windows/Linux), or `IntelliJ IDEA` > `Preferences...` (macOS) +
.. Select `Build, Execution, Deployment` > `Build Tools` > `Gradle` > `Runner` +
.. Select `Gradle Test Runner` under `Run test using`
...
See <<UsingGradle#rendering-asciidoc-files, UsingGradle.adoc>> to learn how to preview `.adoc` files locally.
...
.  Make sure you have set up GitHub Pages as described in <<DeveloperGuide#publishing-documentation, Publishing Documentation>>.
.  Using Chrome, go to the URL of the documentation file. e.g. For <<UserGuide#, UserGuide.adoc>>, the URL will be `\https://<your-username-or-organization-name>.github.io/addressbook-level4/docs/UserGuide.html`.
