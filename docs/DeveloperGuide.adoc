...
`UndoableCommand` implements the https://www.tutorialspoint.com/design_pattern/template_pattern.htm[template pattern]. The template pattern allows users of `UndoableCommand` to continue using the `execute()` method just like any other `Command`, while allowing `UndoableCommand` to inject additional behaviors to `execute()` (such as saving the state of the address book before execution) that will be automatically inherited by subclasses of `UndoableCommand`. Subclasses of `UndoableCommand` will now override `executeUndoableCommand()` instead of `execute()` as a result.
