...
Before `UndoableCommand`, commands were implemented this way:
[source,java]
----
...
With the extra layer, the commands are now implemented this way:
[source,java]
----
...
The user now decides to execute a new command, `delete 5`. As before, `delete 5` will be pushed into the `undoStack`. This time the `redoStack` is no longer empty. It will be purged as it no longer make sense to redo the `add n/David` command (this is the behavior that most modern desktop applications follow).
